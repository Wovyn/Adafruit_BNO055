...
/**************************************************************************/
/*!
    @brief  Initialize default Wire and call setupHW()
*/
/**************************************************************************/
bool Adafruit_BNO055::begin(adafruit_bno055_opmode_t mode)
{
   ...
   #ifdef ESP8266
     _hardPort->setClockStretchLimit(1000); // Allow for 1000us of clock stretching
   #endif
...
/**************************************************************************/
/*!
    @brief  Initialize TwoWire and call setupHW()
*/
/**************************************************************************/
bool Adafruit_BNO055::begin(TwoWire &wirePort, adafruit_bno055_opmode_t mode)
{
...
/**************************************************************************/
/*!
    @brief  Initialize SoftwareWire and call setupHW()
*/
/**************************************************************************/
bool Adafruit_BNO055::begin(SoftwareWire& wirePort, adafruit_bno055_opmode_t mode)
{
   ...
   // BNO055 clock stretches for 500us or more!
   #ifdef ESP8266
     _softPort->setClockStretchLimit(1000); // Allow for 1000us of clock stretching
   #endif
   #endif
...
*/
/**************************************************************************/
bool Adafruit_BNO055::setupHW(adafruit_bno055_opmode_t mode)
{
...
bool Adafruit_BNO055::write8(adafruit_bno055_reg_t reg, byte value)
{
  switch(_wireType)
    {
      case(HARD_WIRE):
        _hardPort->beginTransmission(_address);
        #if ARDUINO >= 100
          _hardPort->write((uint8_t)reg);
          _hardPort->write((uint8_t)value);
        #else
          _hardPort->send(reg);
          _hardPort->send(value);
        #endif
      ...
      case(SOFT_WIRE):
        #ifdef SoftwareWire_h
        _softPort->beginTransmission(_address);
        #if ARDUINO >= 100
          _softPort->write((uint8_t)reg);
          _softPort->write((uint8_t)value);
        #else
          _softPort->send(reg);
          _softPort->send(value);
        #endif
  ...
  switch(_wireType)
    {
      case(HARD_WIRE):
        ...
        #endif
        _hardPort->endTransmission();
        _hardPort->requestFrom(_address, (byte)1);
        #if ARDUINO >= 100
          value = _hardPort->read();
        #else
          value = _hardPort->receive();
        #endif
        break;
      ...
      case(SOFT_WIRE):
        ...
        #endif
        _softPort->endTransmission();
        _softPort->requestFrom(_address, (byte)1);
        #if ARDUINO >= 100
          value = _softPort->read();
        #else
          value = _softPort->receive();
        #endif
        #endif
        break;
    }
  ...
  switch(_wireType)
    {
      case(HARD_WIRE):
        ...
        #endif
        _hardPort->endTransmission();
        _hardPort->requestFrom(_address, (byte)len);

        for (uint8_t i = 0; i < len; i++)
        {
          #if ARDUINO >= 100
            buffer[i] = _hardPort->read();
          #else
            buffer[i] = _hardPort->receive();
          #endif
        }
        break;
      ...
      case(SOFT_WIRE):
        ...
        #endif
        _softPort->endTransmission();
        _softPort->requestFrom(_address, (byte)len);

        for (uint8_t i = 0; i < len; i++)
        {
          #if ARDUINO >= 100
            buffer[i] = _softPort->read();
          #else
            buffer[i] = _softPort->receive();
          #endif
        }
        #endif
        break;
